...
public class GetCarpool extends ActionSupport {
	public void get() throws ClassNotFoundException, SQLException, IOException{
		...
		if(type==1){//local
			String sql = "select * from local where id="+trip_id;
			...
			if(info.next()){
				...
				detail.put("status",info.getInt("cartype"));
		...
		}else if(type==2){//nationdri
			String sql = "select * from nation where id="+trip_id;
			ResultSet info = stmt.executeQuery(sql);
			if(info.next()){
